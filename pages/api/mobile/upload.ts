  ...
  try {
    const jsonResponse = await handleUpload({
      ...
      onBeforeGenerateToken: async (pathname, clientPayload) => {
        if (!pathname.startsWith("dalimss-mobile/")) {
        ...
        const isCaptions = payload.kind === "captions";

        return {
          allowedContentTypes: isCaptions ? ["text/vtt", "text/plain"] : isCover
          ...
          maximumSizeInBytes: isCaptions ? 1024 * 1024 : isCover
            ? 10 * 1024 * 1024
    ...
    console.error("Mobile upload authorization failed:", error);

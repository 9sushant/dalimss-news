...
export default function EditStoryPage({ story: initialStory }: Props) {
  ...
  if (!session) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p>Please sign in to edit stories.</p>
      </div>
    );
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      for (let i = 0; i < pages.length; i++) {
        const page = pages[i];
        setUploadProgress(`Processing page ${i + 1} of ${pages.length}...`);
      ...
      const res = await fetch(`/api/stories/${initialStory.slug}`, {
        method: "PUT",
      ...
      router.push(`/stories/${initialStory.slug}`);
    } catch (err: any) {
      ...
      <div className="max-w-3xl mx-auto px-4">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <Link href={`/stories/${initialStory.slug}`} className="text-gray-600 hover:underline">← Back</Link>
          </div>
        </div>
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-xl shadow-lg p-6 space-y-6">
          ...
          <div>
            ...
            <label className="flex flex-col items-center justify-center w-full h-40 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-red-500 bg-gray-50 transition-colors overflow-hidden">
              {coverImage ? (
          ...
          <div>
            <div className="flex items-center justify-between mb-3">
              <label className="block text-sm font-medium text-gray-700">Story Pages ({pages.length})</label>
              <button
                type="button"
                onClick={addPage}
              ...
              {pages.map((page, idx) => (
                <div key={idx} className="border border-gray-200 rounded-lg p-4 bg-gray-50">
                  ...
                  <div className="space-y-3">
                    {/* Page Image */}
                    <label className="flex flex-col items-center justify-center w-full h-32 border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-red-500 bg-white transition-colors overflow-hidden">
                      {page.imageUrl ? (
